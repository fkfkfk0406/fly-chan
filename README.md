...
|---|---|---|
| 🍓 딸기 | 당 GRN (배고플수록 셈) | MN9 120 Hz 안팎 → 잘 먹음. 배부르면 반응이 약해 "배불러~" |
| 🍯 꿀 | 인두 당 GRN | MN9 90 Hz 안팎 → 잘 먹음 |
| 💧 물 | 물 GRN (200 Hz) | MN9 70 Hz 안팎 → 마심 |
| 🥨 짠 과자 | Ir94e | MN9 0 Hz → 맛만 보고 안 먹음. 당과 같이 있으면 식욕까지 눌림 |
...
**하트와 상점**
- 하트는 함께 있는 동안에도, 자리를 비운 동안에도 모입니다(최대 12시간치). 애정이 깊고 방이 깨끗할수록 빨리 모여요. 돌아오면 "기다리면서 하트 N개 모았어!"로 받습니다.
- 돌봄(밥·쓰담·청소·대화)으로도 조금씩 얻고, 하루 40개까지만 쌓입니다.
- 간식은 재고가 있어서 떨어지면 상점에서 사야 해요(딸기 5, 꿀 9, 물 3, 짠 과자 4, 쓴 버섯 2).
...
- **한 번에 하나만 맛봅니다.** 두 가지 맛을 동시에 넣으면(특히 Ir94e + 쓴맛) 전뇌가 폭주해서, 입에 닿은 것 중 가장 가까운 하나만 자극합니다(`scripts/snack-safety.ts`).
- 세기 제한: 물 GRN은 100 Hz에서 반응이 없어 200 Hz, Ir94e는 200 Hz에서 폭주해 100 Hz로 씁니다(`scripts/taste_probe.ts`).
